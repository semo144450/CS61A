...
def digit_fn(digit):
    ...
    assert isinstance(digit, int) and digit < 10
...
def hefty_hogs(player_score, opponent_score):
    ...
    if opponent_score == 0:
        ...
        while 1:
            player_score = digit_fn(digit)(player_score)
            opponent_score //= 10
            if opponent_score == 0:
                break
            digit = opponent_score % 10
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            num_rolls = strategy0(score0, score1)  # Player 0's strategy
            score0 += take_turn(num_rolls, score0, score1, dice, goal)
            score0 += hog_pile(score0, score1)
        else:
            num_rolls = strategy1(score1, score0)  # Player 1's strategy
        ...
        # Check if any player has reached the goal score
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    message = f'Player {leader} takes the lead by {sub}'
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    max_average = make_averaged(roll_dice, 1000)(10, dice) 
    max_num = 10
    for i in range(9, 0, -1):
        test = make_averaged(roll_dice, 1000)(i, dice) 
        if test >= max_average:
            max_average = test
            max_num = i
    return max_num
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if (hog_pile(score, opponent_score) > 0) or (hefty_hogs(score, opponent_score) >= threshold):
        return 0
    else:
        return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
